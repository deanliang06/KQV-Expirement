...
class CNNAutoencoder(nn.Module):
    def __init__(self, ndim=768, d=64):
        super().__init__()
    ...
    def forward(self, x):
        x = self.mirror(x.view(1, self.ndim, self.ndim)).view(1, 1, self.ndim + 64*2, self.ndim + 64*2)
        ...
        first_up_map = self.first_up(torch.cat((bottom_map, crop(fourth_map, 0, 0, 96, 96)), 1))
        third_map = self.third_res_pre_conv(third_map)
        second_up_map = self.second_up(torch.cat((first_up_map, crop(third_map, 0, 0, 188, 188)), 1))
        if self.d <= 188:
